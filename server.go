...
func (s *Server) addTransfer(origin, destination *Account, amount uint64) (id uint64, err error) {
...
func (s *Server) exchangeAmount(originAcc, destinationAcc, amount uint64) error {
	//retrieving accounts
	origin, ok := s.accountStore.dataStorage[originAcc]
	destination, ok2 := s.accountStore.dataStorage[destinationAcc]
	if !ok || !ok2 {
		return errors.New("impossible to retrieve accounts")
	}
	//modifying balances
	origin.Balance, destination.Balance = origin.Balance-amount, destination.Balance+amount
	//persisting new data
	s.accountStore.dataStorage[originAcc] = origin
	s.accountStore.dataStorage[destinationAcc] = destination
...
func NewServer(as *AccountStore, ts *TransferStore) *Server {
